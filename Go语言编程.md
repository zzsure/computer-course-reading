...
- 流程控制语句一般起3个作用
  ...
  3. 跳转，即根据条件返回到某执行序列
- Go语言支持如下的几种流程控制语句
  1. 条件语句，对应的关键字：if、else和else if
  2. 选择语句，对应的关键字：switch、case好select
  3. 循环语句，对应的关键字：for和range
  4. 跳转语句，对应的关键字：goto

### 2.4.1 条件语句
- 条件语句不需要使用括号将条件包含起来()
- 无论语句内有几条语句，花括号{}都是必须存在的
...
- 条件表达式不限制为常量或者整数
- 单个case中，可以出现多个结果选项
- 与C语言等规则相反，Go语言不需要break明确退出一个case
- 只有在case中明确添加fallthrough关键字，才会继续执行紧跟的下一个case
- 可以不设定switch之后的条件表达式，在此情况下，整个switch结构与多个if...else的逻辑作用等同

### 2.4.3 循环语句
- 左花括号{必须与for处于同一行
- Go语言中的for循环与C语言一样，都允许在循环条件中定义和初始化变量，唯一的区别是，Go语言不支持以逗号为间隔的多个赋值语句，必须使用平行复制的方式来初始化多个变量
- Go语言的for循环同样支持continue和break来控制循环，提供了一个更高级的break可以选择而终端哪一个循环

### 2.4.4 跳转语句
- goto语句非常简单，就是跳转到本函数的某个标签

## 2.5 函数
- 函数基本组成：关键字func、函数名、参数列表、返回值、函数体和返回语句

### 2.5.1 函数定义
- 如果参数列表中多个相邻的参数类型相同，可以省略前面的类型声明
- 返回列表多个返回值的类型相同，也可以省略前面的声明

### 2.5.2 函数调用
- 函数首字母大写可以被包外调用

### 2.5.3 不定参数
- 不定参数是指参数传入的参数个数为不定数量，函数定义args ...int
- 语法糖这种方式避免了传参的时候要建立Slice数组
- 可以传递Slice片段，myfunc(args[1:]...)
- 传递人员我类型的不定参数，args ...interface{}

### 2.5.4 多返回值
- 返回值命名后，它们的值在函数开始的时候被自动初始化为空，return就返回对应的返回值变量的值

### 2.5.5 匿名函数与闭包
- 函数可以像普通变量一样被传递或使用，支持随时在代码里面定义匿名函数
- 匿名函数由一个不带函数名的函数声明和函数体组成，匿名函数可以直接赋值给一个变量或者直接运行
- f := func(x, y int) int { return x+y }    func(ch chan int) { ch <- ACK } (reply_chan)，花括号后直接跟从参数列表表示函数调用
- Go的匿名函数是一个闭包，闭包的实现确保只要闭包还被使用，那么被闭包引用的变量就会一直存在

## 2.6 错误处理

### 2.6.1 error接口
- 错误的标注处理模式，即error接口，type error interface { Error() string }
- 大多数函数，如果返回错误，一般将error作为多种返回值的最后一个

### 2.6.2 defer
- 下面代码发生异常，都会调用defer的语句
- 可以在defer加一个匿名函数   defer func() { // 做复杂的清理操作 } ()
- 如果有多个defer语句，遵循先进后出的原则

### 2.6.3 panic()和recover()
- func panic(interface{})，func recover() interface{}
- 调用panic后，正常函数的执行流程立即终止，但函数中之前使用defer关键字延迟的语句将正常展开执行，之后该函数将返回到调用函数，并导致逐层向上执行panic流程，直至所属的goroutine中所有正在执行的函数被终止
- recover()函数用于终止错误处理流程，一般情况下，recover()应该在一个使用defer关键字的函数中执行有效截取错误处理流程。如果没有在发生异常的goroutine中明确调用恢复过程，会导致该goroutine所属的进程打印异常信息后直接退出。
